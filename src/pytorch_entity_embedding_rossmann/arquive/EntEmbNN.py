...
class EntEmbNNRegression(nn.Module):
    ...
    def split_train_test(self):
        ...
        _, _ = check_X_y(self.X, self.y.values.ravel())
    ...
    def init_embeddings(self):
        ...
        for f in self.cat_features:
            ...
            self.embeddings[f] = nn.Embedding(vocab_size, emb_dim)
    ...
    def get_y_norm_constants(self):
        if self.y_range is None:
            ...
            if not (self.X_y_test is None):
                self.y_max = max(self.X_y_test[1].max(), self.y_max)

                self.y_min = min(self.X_y_test[1].min(), self.y_min)
        else:
    ...
    def format_X(self, X):
        ...
        """
        return pd.concat(
            [X[self.cat_features].apply(lambda x: x.cat.codes), X[self.num_features]],
            axis=1,
        )
    ...
    def forward(self, x):
        ...
        for hl_it, (hl_name, hl) in enumerate(self.dense_layers.items()):
            is_inner_layer = hl_it <= (len(self.dense_layers) - 2)
            x = hl(x)
            if is_inner_layer:
                #                x = F.dropout(
                #                    x,
    ...
    def eval_model(self):
        ...
        train_y_pred = self.predict(self.X_train, log_scale=False, format_X=False)

        self.format_y_inv(train_y_pred)

        loss_train = pd.Series(self.y_train - train_y_pred).abs().mean()

        test_y_pred = self.predict(self.X_test, log_scale=False, format_X=False)

        loss_test = pd.Series(self.y_test - test_y_pred).abs().mean()

        msg = "\t[%s] MAPE train:%s MAPE test:%s"

        msg_params = (self.epoch_cnt, round(loss_train, 5), round(loss_test, 5))
...
def test():
    ...
    print(
        "MAPE: %s" % eval_utils.MAPE(y_true=y_test.values.flatten(), y_pred=test_y_pred)
    )

    eval_utils.eval_regression(y_true=y_test, y_pred=test_y_pred)
