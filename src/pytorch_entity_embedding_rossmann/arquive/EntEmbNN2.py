...
class EntEmbNNRegression(nn.Module):
    ...
    def X_transform(self, X):
        ...
        for c in self.cat_features:
            ...
            is_unknown_codes = ~codes.isin(self.labelencoders[c].classes_)
    ...
    def get_embeddings(self):
        ...
        for c in self.cat_features:
            ...
            categories = pd.Series([x for x in categ_names], index=categ_codes.values)
            ...
            emb = pd.DataFrame(emb.numpy(), index=categories.values)
    ...
    def eval_model(self):
        ...
        loss_train = (
            pd.Series(self.y_transform(self.y_train) - self.y_transform(train_y_pred))
            .abs()
            .mean()
        )
        ...
        loss_test = (
            pd.Series(self.y_transform(self.y_test) - self.y_transform(test_y_pred))
            .abs()
            .mean()
        )
